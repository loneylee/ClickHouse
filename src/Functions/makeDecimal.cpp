...
#include <Interpreters/convertFieldToType.cpp>
...
namespace
{
    ...
    private:
        ...
        static bool convertDecimalsFromIntegerImpl(const Field & field, ToNativeType & result)
        {
            Field f = convertNumericTypeImpl<FromNativeType, ToNativeType>(field);
            if (f.isNull())
            {
                if constexpr (ConvertExceptionMode::Throw == exception_mode)
                    throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "{} convert overflow", field.getTypeName());
                else
                    return false;
            }
            result = static_cast<ToNativeType>(f.get<ToNativeType>());
            return true;
        }
    };
